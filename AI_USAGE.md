...
### Discarded PDF packaging experiment

The candidate initially asked for the finalized Markdown report to be converted into a PDF suitable for an email attachment. A dependency-free Markdown renderer and the installed headless Microsoft Edge browser were used and verified. The candidate later decided that PDF artifacts were unnecessary, so the generated files and renderer were removed while `REPORT.md` remained the submission report.
...
| Put every `.env` value in the email body | Changed for security | The runtime URL and publishable key can be provided, but the database password and personal Supabase access token are privileged provisioning credentials and are not needed for ordinary application review. |
...
A temporary PDF was checked for a valid header, non-zero size, a six-page page tree, balanced Markdown code fences, and absence of supplied demo passwords before the candidate decided not to include PDF artifacts. The generated PDFs and renderer were then removed from the final project tree.
